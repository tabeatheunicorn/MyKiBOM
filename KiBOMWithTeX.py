...
def makeTeXFileFromFormatedBuffer(tex_file, *args):
    ...
    Latex.setBackgroundPicture(tex_file, path_to_AI_logo)
    latex_string = ''
    for arg in args:
...
# instead
print(sys.version)
#This is copy-pasted from bom_csv_sorted_by_ref.py
...
name_of_document = 'Version' + str(date_of_netlist).replace('.', '_').replace(' ', '_').replace(':','_')  + '.tex'

try:
    doku_directory = sys.argv[2] # will write into project folder
